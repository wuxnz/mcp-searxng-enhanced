...
class Tools:
    class Valves(BaseModel):
        ...
        def check_page_limits(self) -> 'Valves':
            if self.SCRAPPED_PAGES_NO < self.RETURNED_SCRAPPED_PAGES_NO:
                raise ConfigurationError(
                    f"SCRAPPED_PAGES_NO ({self.SCRAPPED_PAGES_NO}) must be greater than or equal to "
                    f"RETURNED_SCRAPPED_PAGES_NO ({self.RETURNED_SCRAPPED_PAGES_NO})"
                )
            return self
    ...
    def __init__(self, send_notification_func: Callable[[str, Any], Awaitable[None]]):
        ...
        script_defaults = self.Valves().model_dump()
